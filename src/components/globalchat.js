...
export default function Globalchat() {
  ...
  const { loading, data } = useQuery(GET_MESSAGE_QUERRY, {onError:((err)=>{if (err.networkError) {
  ...
  const [send, { msgSendingLoading }] = useMutation(SEND_MESSAGE, {
    update() {
          ...
          Object.values(messages).map((value) => (
            <>
              {user && value.username === user.username ? (
                ...
                <>
                  <div className="message-titles">{value.username}</div>
                  <div className="back">
